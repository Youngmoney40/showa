...
export default function ChannelAdminScreen({ route, navigation }) {
  ...
  const renderReactionPicker = (messageId) => {
    ...
    return (
      ...
      >
        <TouchableWithoutFeedback onPress={hideReactionPicker}>
          <View style={[styles.reactionModalOverlay, { backgroundColor: 'rgba(0,0,0,0.5)' }]}>
            <View style={[styles.reactionModalContainer, { backgroundColor: colors.card }]}>
              <View style={styles.reactionPicker}>
                <EmojiSelector
                  onEmojiSelected={(emoji) => {
                    handleReaction(reactionPickerForMessage, emoji);
  ...
  const renderMessage = ({ item }) => {
    ...
    if (item.is_deleted) {
      return (
        <View style={styles.messageContainer}>
          <Text style={[styles.deletedMessage, { color: colors.textTertiary }]}>
    ...
    return (
      <View style={[
        ...
        <View style={[
          styles.messageContainer,
          isMyMessage ? [styles.myMessage, { backgroundColor: colors.primary }] : [styles.otherMessage, { backgroundColor: colors.card }],
          item.is_channel_post && styles.channelPost
        ]}>
          {isMyMessage && (
            <TouchableOpacity 
          ...
          >
            ...
            {item.file && (
              <TouchableOpacity
                style={[styles.fileContainer, { backgroundColor: colors.backgroundSecondary }]}
                onPress={() => Linking.openURL(item.file).catch(() => alert('Cannot open file'))}
              >
                <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            ...
            {item.emoji && <Text style={styles.emojiMessage}>{item.emoji}</Text>}
            {item.content && <Text style={[styles.messageText, { color: isMyMessage ? '#fff' : colors.text }]}>{item.content}</Text>}
            {item.content && <Text style={[styles.messageText, { fontSize: 13, color: isMyMessage ? 'rgba(255,255,255,0.7)' : colors.textTertiary, alignSelf: 'flex-end' }]}>{timeString}</Text>}
          </TouchableOpacity>
          ...
          {item.reactions?.length > 0 && (
            <View style={[
              ...
              <View style={[styles.reactionsBubble, { backgroundColor: isDark ? colors.surfaceSecondary : 'rgba(0,0,0,0.08)' }]}>
                {item.reactions.slice(0, 3).map((reaction, index) => (
                  <Text key={index} style={styles.reactionEmoji}>
                    {reaction.emoji}
                  </Text>
                ...
                {item.reactions.length > 3 && (
                  <Text style={[styles.reactionCount, { color: colors.textTertiary }]}>
                    +{item.reactions.length - 3}
                  </Text>
                )}
              </View>
  ...
  if (isLoading) {
    return (
      <View style={[styles.loadingContainer, { backgroundColor: colors.background }]}>
        ...
        <Text style={{ color: colors.text }}>Loading channel please wait...</Text>
      </View>
    );
  }
      ...
      >
        {isAndroid15 ? (
          <View style={styles.container}>
            ...
            >
              <View style={styles.headerContent}>
                ...
                <View style={styles.headerProfile}>
                  ...
                  <View>
                    <Text style={styles.headerName}>{name}</Text>
                    <Text style={[styles.followersText, { color: 'rgba(255,255,255,0.8)' }]}>
                      {followers} followers
                    </Text>
                  </View>
                </View>
              ...
              <TouchableWithoutFeedback onPress={closeMenu}>
                <View style={[styles.menuOverlay, { backgroundColor: 'rgba(0,0,0,0.5)' }]}>
                  <View style={[styles.menuContainer, { backgroundColor: colors.card }]}>
                    ...
                    >
                      <Text style={[styles.menuItemText, { color: colors.text }]}>Share Channel</Text>
                    </TouchableOpacity>
                    <TouchableOpacity 
                      style={styles.menuItem}
                      onPress={() => {
                    ...
                    >
                      <Text style={[styles.menuItemText, styles.leaveText, { color: colors.error || '#F44336' }]}>Delete Channel</Text>
                    </TouchableOpacity>
            ...
            {messages.length === 0 && pendingMessages.length === 0 ? (
              <View style={[styles.emptyChannelContainer, { backgroundColor: 'transparent' }]}>
                <Text style={[styles.emptyChannelText, { 
                  color: colors.textSecondary,
                  backgroundColor: colors.card,
                  borderColor: colors.border
                }]}>
                  No posts yet. Share something with your followers!
                </Text>
              </View>
            ) : (
              <FlatList
            ...
            <View style={[
              ...
              <TouchableOpacity onPress={() => setModalVisible(true)} style={styles.attachButton}>
                <Icon name="attach-file" size={27} color={colors.primary} />
              </TouchableOpacity>
              <TextInput
                ref={inputRef}
                style={[styles.input, { 
        ...
        <Modal transparent visible={isImageLoading} onRequestClose={() => {}}>
          <View style={[styles.loadingModalOverlay, { backgroundColor: 'rgba(0,0,0,0.5)' }]}>
        ...
        <Modal
          ...
          onRequestClose={() => setFullscreenImage(null)}
        >
          <View style={[styles.fullscreenImageOverlay, { backgroundColor: 'rgba(0,0,0,0.9)' }]}>
            <TouchableWithoutFeedback onPress={() => setFullscreenImage(null)}>
              <Image 
                source={{ uri: fullscreenImage }} 
                style={styles.fullscreenImage} 
        ...
        <Modal transparent visible={imagePreviewModalVisible} onRequestClose={() => setImagePreviewModalVisible(false)}>
          <View style={[styles.imagePreviewModalOverlay, { backgroundColor: '#1A1A1A' }]}>
            <View style={styles.imagePreviewModalContent}>
              <View style={styles.imagePreviewHeader}>
                <TouchableOpacity
                  onPress={() => {
        ...
        <Modal transparent visible={emojiPickerVisible} onRequestClose={() => setEmojiPickerVisible(false)}>
          <TouchableWithoutFeedback onPress={() => setEmojiPickerVisible(false)}>
            <View style={[styles.modalOverlay, { backgroundColor: 'rgba(0,0,0,0.5)' }]}>
              <View style={[styles.emojiPickerContainer, { backgroundColor: colors.card }]}>
                <EmojiSelector onEmojiSelected={selectEmoji} />
              </View>
            </View>
          </TouchableWithoutFeedback>

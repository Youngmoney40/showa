...
// Quick actions in a grid row
const QUERY_SUGGESTIONS = [
  { 
    title: "Social Media", 
    icon: "trending-up", 
    prompt: "Help me create a social media strategy to grow my business."
  },
  { 
    title: "Marketing", 
    icon: "trending-up", 
  ...
  { 
    title: "Money Ideas", 
    icon: "bulb", 
    prompt: "Give me creative content ideas for my social media posts."
  },
  { 
    title: "Business", 
    icon: "megaphone", 
    prompt: "Write compelling ad copy for my products/services."
  },
  { 
    title: "Money Tip", 
    icon: "person", 
    prompt: "Help me optimize my social media profiles."
  },
  { 
    title: "Friendship", 
    icon: "images", 
    prompt: "Generate creative captions for my photos."
  },
  { 
    title: "Engagement", 
    icon: "heart", 
    prompt: "Share tips to increase engagement on my posts."
  },
  { 
    title: "Hashtags", 
    icon: "pound", 
    prompt: "Suggest trending hashtags for my niche."
  },
  { 
    title: "Growth Tips", 
    icon: "heart", 
    prompt: "Tips to increase engagement on posts."
  },
  { 
    title: "Bio & Profile", 
    icon: "person", 
    prompt: "Write a professional bio for my business."
  },
...
const AIChatScreen = ({ route, navigation }) => {
  ...
  const checkBackendHealth = async () => {
    try {
      const response = await axios.get(API_ENDPOINTS.GEMINI_HEALTH, { timeout: 3000 });
      if (response.data.status === 'healthy') {
        setModelInfo(prev => ({ ...prev, status: 'online' }));
      }
    } catch (error) {
  ...
  // Split suggestions into 2 rows for grid
  const splitIntoRows = (data, itemsPerRow = 3) => {
    const rows = [];
  ...
  const suggestionRows = splitIntoRows(QUERY_SUGGESTIONS, 3);

  return (
    <SafeAreaView style={styles.container}>
      <StatusBar barStyle="light-content" backgroundColor="#0A0A0C" />
      
      {/* Header - Centered like DeepSeek */}
      <View style={styles.header}>
        ...
        <View style={styles.headerCenter}>
          <Text style={styles.headerTitle}>showa Ai</Text>
        </View>
        
        <TouchableOpacity style={styles.clearButton} onPress={clearChat}>
          <Icon name="trash-outline" size={20} color="#6B7280" />
        </TouchableOpacity>
      </View>
      ...
      {showSuggestions && messages.length === 0 ? (
        ...
        >
          ...
          >
            ...
            <View style={styles.welcomeSection}>
              <Text style={styles.greetingText}>
                <Text style={styles.userNameHighlight}>Showa Ai</Text>
              </Text>
              <Text style={styles.subGreeting}>
                How can I assist you today?
              </Text>
            ...
            {/* Search Input - Clean like DeepSeek */}
            <View style={styles.searchContainer}>
              <View style={styles.searchWrapper}>
            ...
            <View style={styles.chatInputWrapper}>
              ...
              >
                <Icon 
                  name="send" 
                  size={22} 
                  color={message.trim() && !loading ? '#3B82F6' : '#4B5563'} 
                />
              </TouchableOpacity>
            </View>
          </View>

...
// Handle new message from WebSocket - store update for later if not focused
const handleNewMessage = (data) => {
  const { chat_id, chat_type, content, timestamp, sender_name, account_mode, group_slug } = data;
  
  // For groups, use group_slug as the identifier
  // For single chats, use chat_id
  let chatIdentifier;
  if (chat_type === 'group') {
    chatIdentifier = group_slug || chat_id; // Use group_slug, fallback to chat_id
  } else {
    chatIdentifier = chat_id;
  }
  ...
  // Find the chat in the current list
  const existingIndex = currentList.findIndex(chat => 
    chat.id === chatIdentifier && chat.type === chat_type
  );
  ...
  if (existingIndex !== -1) {
    ...
    const isFromCurrentUser = sender_name === userData?.name;
    const newUnread = isFromCurrentUser ? currentUnread : currentUnread + 1;
...
const processNewChatsSilently = (newChats, mode) => {
  ...
  const processedChats = filteredChats.map(newChat => {
    // For groups, use group_slug as the identifier
    // For single chats, use the participant ID or chat id
    let chatIdentifier;
    if (newChat.type === 'group') {
      chatIdentifier = newChat.group_slug || newChat.id;
    } else if (newChat.type === 'single') {
      chatIdentifier = newChat.participants?.find(id => id !== newChat.current_user_id) || newChat.id;
    } else {
      chatIdentifier = newChat.id;
    }
    ...
    return {
      ...
      unread_count: actualUnread,
      name: newChat.name || 'Unknown',
      content: newChat.content || (newChat.type === 'group' ? 'No messages yet' : '[media]'),
      time: newChat.timestamp ? new Date(newChat.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '',
      avatar: newChat.avatar ? `${API_ROUTE_IMAGE}${newChat.avatar}` : null,
      type: newChat.type,
      members_count: newChat.members_count,
      receiverId: newChat.type === 'single' ? chatIdentifier : null,
      group_slug: newChat.group_slug || null,
      timestamp: newChat.timestamp,
      has_messages: newChat.has_messages !== false,
    };
  });
...
const forceRefreshChats = useCallback(async () => {
  console.log('[Chat] Force refreshing chats...');
  await fetchAllChats();
}, [fetchAllChats]);

// Expose this function via useEffect or pass it to navigation params
useEffect(() => {
  // Listen for navigation events to refresh when returning from chat screenfrt
  const unsubscribe = navigation.addListener('focus', () => {
    console.log('[Chat] Screen focused, refreshing...');
    fetchAllChats();
  });
...
//       console.log('fetch business chat',fetchBusinessChats)
...
//     unread_count: actualUnread,
//     name: chat.name || 'Unknown',
//     content: chat.content || (chat.type === 'group' ? 'No messages yet' : '[media]'),
...
//     group_slug: chat.group_slug || null,
//     timestamp: chat.timestamp,
//     has_messages: chat.has_messages !== false, // Default to true if not specified
//   };
// });
...
const fetchBusinessChats = async () => {
  ...
  try {
    ...
    // DON'T filter out chats with no messages - keep all chats
    const filteredChats = response.data.chats.filter(chat =>
      chat.type !== 'channel' 
    );

    console.log('fetch business chat', filteredChats)
    ...
    const chats = uniqueChats.map((chat) => {
      ...
      return {
        ...
        unread_count: actualUnread,
        name: chat.name || 'Unknown',
        // Fix: Use a default message for groups with no messages
        content: chat.content || (chat.type === 'group' ? 'No messages yet' : '[media]'),
        // Fix: Handle missing timestamp
        time: chat.timestamp ? new Date(chat.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '',
        avatar: chat.avatar ? `${API_ROUTE_IMAGE}${chat.avatar}` : null,
        type: chat.type,
        members_count: chat.members_count,
  ...
  if (mode === 'all') {
    ...
    currentList = businessChatList;  // ✅ This includes groups
  } else {
    currentList = personalChatList;
  }
  setFilteredChatList(currentList);

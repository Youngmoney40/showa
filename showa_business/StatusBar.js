...
//     // Use Alert.alert like your working version
...
//   // Quick-tap reactions shown in the reply modal. Tapping one sends that emoji
//   // straight to chat as a message (not the status "react" API — that stays on
//   // the main viewer page only, per the heart-outline button there).
//   const QUICK_REACTIONS = ['❤️', '👍', '😂', '😮', '😢'];

//   // Shared sender: posts a chat message (with the status context + optional
//   // status image attached) to whoever owns the status currently open in the
//   // reply modal. Used by both the typed reply and the one-tap quick reactions.
...
//   // Tap a quick-reaction emoji: send it immediately as a chat message, no need
//   // to type anything or press Send.
...
//     // Fetch details for each viewer ID
...
//   console.log('👤 Rendering viewer:', item);
  
//   // The response has the user data nested inside a 'user' object
//   const userData = item.user || item; // Fallback if item is already the user object
...
// const fetchViewerDetails = async (viewerIds) => {
//   try {
//     const token = await AsyncStorage.getItem('userToken');
//     const viewerPromises = viewerIds.map(async (viewerId) => {
//       try {
//         const response = await axios.get(`${API_ROUTE}/users/${viewerId}/profile/`, {
//           headers: {
//             Authorization: `Bearer ${token}`,
//             'Content-Type': 'application/json',
//           },
//         });
//         console.log('📱 Viewer data fetched:', response.data);
//         return response.data;
//       } catch (error) {
//         console.error('Error fetching viewer:', viewerId, error);
//         return { id: viewerId, name: `User ${viewerId}`, phone: viewerId };
//       }
//     });
...
//         {/* Combined Viewers and Delete Section */}
//         {/* Combined Viewers and Delete Section */}
...
//   //   // FIX: previously used Alert.alert here. Since a full-screen statusBarTranslucent
//   //   // Modal is already open underneath, the native Alert can fail to stack above it on
//   //   // some devices — the tap then falls through and closes the whole status viewer
//   //   // instead of asking for confirmation. We now use the app's own delete-confirmation
//   //   // Modal (already defined below) which stacks correctly above the viewer.
//   //   // const handleDeletePress = () => {
...
//   // Reply Modal — rendered as a plain function call (see usage below), NOT as a JSX
//   // component tag like <ReplyModal />. Defining it as `const ReplyModal = () => {...}`
//   // inside this component's body used to create a brand-new function/component
//   // reference on every re-render. Since typing in the TextInput triggers a re-render
//   // (via setReplyMessage), React saw a "new" component type on every keystroke and
//   // unmounted + remounted the entire Modal — which is exactly what looked like the
//   // modal "closing and reopening" while typing, and why keystrokes seemed to disappear.
//   // Calling it as `{renderReplyModal()}` avoids that: the JSX returned is reconciled
//   // normally by element type, so the Modal instance (and the TextInput's focus) stays stable.
...
//             {/* Quick reactions — tap any icon to auto-send it as a message */}
...
//   // Snackbar — also rendered as a plain function call (see `{renderSnackbar()}` below)
//   // for the same reason as renderReplyModal above: defining it as a component and using
//   // it as `<Snackbar />` would recreate its identity on every re-render.
...
//     color: 'rgba(255,255,255,0.5)',
//   },
//   // Row of tap-to-send emoji quick reactions shown in the reply modal
//   quickReactionsRow: {
//     flexDirection: 'row',
//     justifyContent: 'space-between',
//     marginBottom: 16,

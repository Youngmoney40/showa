...
const PostDetailBottomSheet = ({ 
  ...
  const handleCommentSubmit = async () => {
    ...
    // Clean the text - remove @mention if it's a reply
    const cleanText = isReply 
      ? newComment.replace(`@${replyToUsername} `, '').trim() 
      : newComment.trim();
    ...
    // Clear input
    setNewComment('');
    const currentReplyId = replyToCommentId;
    setReplyToCommentId(null);
    setReplyToUsername('');

    try {
      const endpoint = isReply
        ? `${API_ROUTE}/comment/${currentReplyId}/reply/`
        : `${API_ROUTE}/posts-comment/${post.id}/comments/`;
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        if (isReply) {
          ...
          if (commentData.comment_count !== undefined) {
            setCommentCount(commentData.comment_count);
            if (onPostUpdate) {
              onPostUpdate(post.id, { comment_count: commentData.comment_count });
            }
          }
        }
      ...
      if (isReply) {
        ...
        if (onPostUpdate) {
          onPostUpdate(post.id, { comment_count: Math.max(0, commentCount - 1) });
        }
      }
      
      Alert.alert('Error', error.response?.data?.error || 'Failed to post comment. Please try again.');
    }
  };
  ...
  const renderCommentItem = useCallback((comment, level = 0) => {
    const isOwnComment = comment.user?.id === currentUserId || comment.user_details?.id === currentUserId;
    const isLiked = likedComments[comment.id] || comment.is_liked || false;
    const replies = comment.replies || [];
        ...
        <View style={styles.commentRow}>
          ...
          >
            <Image
              source={
                commentUserAvatar
                  ? { uri: fixImageUrl(commentUserAvatar) }
                  : require('../assets/images/avatar/blank-profile-picture-973460_1280.png')
              }
              style={styles.commentAvatar}
            />
  ...
  }, [currentUserId, likedComments, handleCommentLike, handleDeleteComment, colors, navigation, fixImageUrl]);
            ...
            >
              <View style={styles.bottomSheetHeader}>
                <View style={styles.bottomSheetHeaderRow}>
                  ...
                  <TouchableOpacity onPress={onClose} style={styles.closeSheetButton}>
              ...
              {imageUrl && (
                <Image
                  source={{ uri: fixImageUrl(imageUrl) }}
                  style={styles.mediaImage}
                  resizeMode="cover"
                />
              )}
...
const ManagePostsScreen = () => {
  ...
  const handleDelete = async (type, id) => {
    try {
      ...
      if (type === 'marketplace') {
        setMarketplacePosts(prev => prev.filter(item => item.id !== id));
      } else if (type === 'tweets') {
        setTweets(prev => prev.filter(item => item.id !== id));
      } else {
        setUserVideos(prev => prev.filter(item => item.id !== id));
      }
    } catch (error) {
      console.error('Delete error:', error);

...
//       setIsLiked(response.data.reaction?.reaction_type === 'like' || false);
//     }
//   } catch (error) {
...
//   // Load data from navigation params
// useEffect(() => {
//   if (postData) {
//     // IMPORTANT: Set isLiked from the passed postData
//     setIsLiked(postData.is_liked || false);
//     setLikeCount(postData.like_count || 0);
...
//   // Add this useEffect — keeps like status accurate no matter where the like happened
// useEffect(() => {
...
//     // DEBUG: check this in your console once to confirm the real shape
//     console.log('like response:', JSON.stringify(response.data));

//     if (response.data) {
//       // Update count from server (source of truth for the number)
//       if (response.data.like_count !== undefined) {
//         setLikeCount(response.data.like_count);
//       }

//       // Only trust server's liked flag if it's explicitly present;
//       // otherwise keep our optimistic value instead of defaulting to false
//       const serverLiked =
...
//       }
//       // if serverLiked is undefined, we just keep newLikedState from the optimistic update
//     }
//   } catch (error) {
//     console.error('Error liking post:', error);
//     // Revert optimistic update on error
...
//   // don't trust the server's is_liked/like_count for a brand-new comment
...
const PostDetailScreen = ({ route, navigation }) => {
  ...
  // SYNC LIKE STATUS ON FOCUS - THIS IS THE KEY FIX
  useFocusEffect(
    useCallback(() => {

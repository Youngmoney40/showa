...
class VideoPrefetchService {
  ...
  async checkAndRefreshCache() {
    try {
      const lastFetch = this.getFromMMKV(LAST_FETCH_KEY);
      if (lastFetch) {
        const timeSinceLastFetch = Date.now() - parseInt(lastFetch);
        if (timeSinceLastFetch > 30 * 60 * 1000) {
          console.log('🎥 Cache is old, refreshing in background...');
  ...
  async prefetchVideos(isBackground = false) {
    try {
      ...
      this.saveToMMKV(VIDEOS_METADATA_KEY, processedVideos);
      this.saveToMMKV(LAST_FETCH_KEY, Date.now().toString());
  ...
  async prefetchThumbnails(videos) {
    ...
    const thumbnailPromises = videos.map(async (video) => {
      ...
      const cached = this.getFromMMKV(cacheKey);
      if (cached) {
      ...
      try {
        ...
        // Save cache info to MMKV
        this.saveToMMKV(cacheKey, {
          path: filePath,
          timestamp: Date.now()
        });
  ...
  async getCachedThumbnailPath(videoId) {
    try {
      const cacheInfo = this.getFromMMKV(`${THUMBNAIL_CACHE_PREFIX}${videoId}`);
      if (cacheInfo) {
  ...
  async getCachedVideoPath(videoId) {
    try {
      const cacheInfo = this.getFromMMKV(`${VIDEO_CACHE_PREFIX}${videoId}`);
      if (cacheInfo) {
        const { path, timestamp } = cacheInfo;
        if (Date.now() - timestamp < this.cacheExpiry) {
          const exists = await RNFS.exists(path);
  ...
  async saveCachedVideoInfo(videoId, path) {
    this.saveToMMKV(`${VIDEO_CACHE_PREFIX}${videoId}`, {
      path,
      timestamp: Date.now(),
    });
  }
  ...
  async getCachedVideos() {
    try {
      const videos = this.getFromMMKV(VIDEOS_METADATA_KEY);
      const lastFetch = this.getFromMMKV(LAST_FETCH_KEY);
  ...
  clearAllMMKVCache() {
    try {
      console.log('🗑️ Clearing all video prefetch MMKV cache...');
      storage.delete(VIDEOS_CACHE_KEY);
      storage.delete(VIDEOS_METADATA_KEY);
      storage.delete(LAST_FETCH_KEY);
      
      // Clear all thumbnail and video cache entries
      const allKeys = storage.getAllKeys();
      allKeys.forEach(key => {
        if (key.startsWith(THUMBNAIL_CACHE_PREFIX) || key.startsWith(VIDEO_CACHE_PREFIX)) {
          storage.delete(key);
        }
      });
      console.log('✅ All video prefetch MMKV cache cleared');
    } catch (error) {

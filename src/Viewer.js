...
const CONNECT_TIMEOUT_MS = 15000;
const WEAK_CONNECTION_LOSS_RATIO = 0.08; // 8% packet loss = "weak"
const OVERLAY_COMMENT_COUNT = 30; // the floating overlay is now scrollable, so keep more history available
...
export default function Viewer({ route, navigation }) {
  ...
  const overlayListRef = useRef(null);
  const isOverlayNearBottomRef = useRef(true); // only auto-scroll the floating overlay if the user hasn't scrolled up to read older comments
  ...
  // Auto-scroll the floating comment overlay to the newest message — but
  // only if the user is already near the bottom, so scrolling up to read
  // older comments doesn't get yanked back down by new messages arriving.
  useEffect(() => {
    if (messages.length > 0 && overlayListRef.current && isOverlayNearBottomRef.current) {
      setTimeout(() => {
        overlayListRef.current?.scrollToEnd({ animated: true });
      }, 50);
    }
  ...
  const handleOverlayScroll = (e) => {
    ...
    isOverlayNearBottomRef.current = distanceFromBottom < 40;
  };
  ...
  // ---- Seed the viewer count to at least 1 (you) once actually connected.
  // Nothing currently sends an authoritative "viewer-count" message, so this
  // — combined with the +1/-1 on viewer-joined/viewer-left below — is what
  // keeps the count accurate the same way the broadcaster tracks it locally. ----
  useEffect(() => {
  ...
  const handleSignaling = async (msg) => {
    ...
    if (msg.type === "broadcaster-answer") {
      ...
      // Kept for forward compatibility in case the backend ever sends this
      // directly — but nothing currently does, which is why viewer-joined /
      // viewer-left below are the actual source of truth for the count.
      if (isMountedRef.current) setViewerCount(msg.count || 0);
    } else if (msg.type === "broadcaster-info") {
      // Sent once on the broadcaster's mount AND again every time a viewer
      // joins, so this reliably reaches late joiners too.
      ...
      // Same +1 logic Broadcaster.js uses for its own viewer count — this is
      // the actual source of truth for the count on this screen too, since
      // nothing sends an authoritative "viewer-count" total.
      if (isMountedRef.current) setViewerCount((prev) => prev + 1);
      setMessages((prev) => [
        ...prev,
        {
          id: `${Date.now()}-${Math.random()}`,
      ...
      if (isMountedRef.current) setViewerCount((prev) => Math.max(0, prev - 1));
      setMessages((prev) => [
        ...prev,
        {
          id: `${Date.now()}-${Math.random()}`,
  ...
  // ---- Bottom sheet controls (mirrors Broadcaster.js's comments sheet) ----
  ...
  const renderMessage = ({ item }) => (
    <View style={[styles.messageBubble, item.isOwnMessage && styles.ownMessageBubble]}>
      <View style={styles.messageHeader}>
        {item.profilePicture ? (
          <Image source={{ uri: `${API_ROUTE_IMAGE}${item.profilePicture}` }} style={styles.messageProfileImage} />
        ) : (
          <View style={[styles.messageProfilePlaceholder, { backgroundColor: getRandomColor(item.username) }]}>
            <Text style={styles.messageProfileInitial}>
              {item.username ? item.username.charAt(0).toUpperCase() : "A"}
      ...
      <KeyboardAvoidingView style={styles.container} behavior={Platform.OS === "ios" ? "padding" : "height"}>
        ...
        <Animated.View style={[styles.header, { opacity: fadeAnim }]}>
          ...
          <View style={styles.streamInfo}>
            ...
            <View style={styles.statsRow}>
              <Icon name="people" size={13} color="#fff" style={{ marginRight: 4 }} />
              <Text style={styles.streamStats}>{formatCount(viewerCount)}</Text>
              <Icon name="heart" size={13} color="#ff375f" style={{ marginLeft: 10, marginRight: 4 }} />
              <Text style={styles.streamStats}>{formatCount(likesCount)}</Text>
            </View>
        ...
        {!showCommentsSheet && messages.length > 0 && (
          <View style={styles.commentsOverlayContainer} pointerEvents="box-none">
            <FlatList
              ref={overlayListRef}
              data={messages.slice(-OVERLAY_COMMENT_COUNT)}
              ...
              <View style={styles.bottomSheetHeader}>
                ...
                <View style={styles.bottomSheetTitleRow}>
                  <Text style={styles.bottomSheetTitle}>
                    Live Chat • {formatCount(viewerCount)} watching
                  </Text>
...
const styles = StyleSheet.create({
  ...
  broadcasterAvatarInitial: { color: "#fff", fontSize: 16, fontWeight: "bold" },
  broadcasterTextCol: { maxWidth: 150 },
  broadcasterName: { color: "#fff", fontSize: 16, fontWeight: "700" },
  liveRow: { flexDirection: "row", alignItems: "center", marginTop: 2 },
  liveBadge: {
    flexDirection: "row",
    alignItems: "center",

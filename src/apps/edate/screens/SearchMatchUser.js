...
export default function IdealMatchScreen({ navigation }) {
  ...
  const fetchUserdata = async () => {
    try {
      ...
      // Get user data from storage after login
      const user_details = await storage.getUser();
      const user_id = user_details.id;
      console.log('User_details', user_id);

      const token = await storage.getAccessToken();
      
      const response = await axios.get(`http://192.168.1.105:8000/api/eprofile/${user_id}/`, {
        headers: {
      ...
      console.log('fetch data successful', response.data);
      
      // Check if user has complete profile data
      if (response.data && response.data.nick_name && response.data.ideal_match) {
        console.log('User has existing profile data, navigating to home');
        setUserHasData(true);
        // Navigate to home screen or main app screen
        navigation.replace('Home'); // Use replace to prevent going back
      } else {
        console.log('User does not have complete profile data, staying on this screen');
        // setUserHasData(false);
        navigation.navigate('ideaMatch')
      }
      
    } catch (error) {
      console.log('Error occurred while fetching data', error);
      // If there's an error (like 404), user doesn't have data
      setUserHasData(false);
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    if (!isLoading && !userHasData) {
      // Only show animations if user doesn't have data
  ...
  // If user has data, show loading briefly then navigate (handled in useEffect)
  if (userHasData) {
    return (
      <LinearGradient
        colors={["#FF3366", "#FF6F00", "#FF3366"]}
      ...
      >
        <View style={styles.loadingContent}>
          ...
          <Text style={styles.loadingText}>Profile found!</Text>
          <Text style={styles.loadingSubtext}>Taking you to your dashboard...</Text>
          <ActivityIndicator size="large" color="#FFF" style={styles.spinner} />
        </View>
      </LinearGradient>
    );
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            <View style={styles.modalHeader}>
              ...
              <Text style={styles.modalTitle}>Search for Match Profile</Text>
            </View>

...
const AccountDashboard = ({ navigation }) => {
  ...
  const clearAllMMKVCache = () => {
    try {
      console.log('🗑️ Clearing all MMKV cache...');
      storage.delete(CACHE_KEYS.BUSINESS_PROFILE);
      storage.delete(CACHE_KEYS.USER_DATA);
      storage.delete(CACHE_KEYS.LAST_FETCH);
      console.log('✅ All MMKV cache cleared');
    } catch (error) {
  ...
  const loadCachedData = async () => {
    try {
      ...
      const cachedUserData = getFromMMKV(CACHE_KEYS.USER_DATA);
      if (cachedUserData) {
        console.log('✅ User data loaded from MMKV cache');
        setUserData(cachedUserData);
        setUserName(cachedUserData.name || cachedUserData.username || 'User');
        setUserEmail(cachedUserData.email || '');
        setUserAvatar(cachedUserData.profile_picture || cachedUserData.image || null);
        setUserId(cachedUserData.id || '');
      }

      // Load business profile status from MMKV cache
      const cachedBusinessProfile = getFromMMKV(CACHE_KEYS.BUSINESS_PROFILE);
      if (cachedBusinessProfile !== null) {
        console.log('✅ Business profile status loaded from MMKV cache');
        setHasBusinessProfile(cachedBusinessProfile);
      }
  ...
  const checkAndRefreshData = async () => {
    try {
      const lastFetch = getFromMMKV(CACHE_KEYS.LAST_FETCH);
      const now = Date.now();
  ...
  const refreshData = async (showLoading = true) => {
    ...
    try {
      ...
      // Fetch user data from AsyncStorage (still needed for auth)
      ...
      if (userDataString) {
        ...
        setUserId(data.id || '');
        // Cache user data in MMKV
        saveToMMKV(CACHE_KEYS.USER_DATA, data);
        console.log('✅ User data cached in MMKV');
      }
      ...
      if (token) {
        try {
          ...
          if (response.status === 200 || response.status === 201) {
            ...
            // Cache business profile status in MMKV
            saveToMMKV(CACHE_KEYS.BUSINESS_PROFILE, hasProfile);
            console.log(`✅ Business profile status: ${hasProfile}`);
          }
        } catch (err) {
          // If API fails, keep cached value
          console.log('⚠️ Business profile fetch failed, using cached value');
        }
      }

      // Update last fetch time in MMKV
      saveToMMKV(CACHE_KEYS.LAST_FETCH, Date.now());
  ...
  const handleShareApp = async () => {
    try {
      await Share.share({
        message: 'Check out Showa - The ultimate social experience! Download now: https://showapp.ng/',
        title: 'Share Showa',
      });
    } catch (error) {
  ...
  const businessItems = [
    ...
    },
    {
      ...
      screen: 'MonetizationRequestForm',
      icon: 'currency-usd',
              ...
              Alert.alert(
                ...
                [
                  ...
                  { 
                    ...
                    style: 'destructive',
                    onPress: async () => {
                      // Clear all MMKV cache on sign out
                      clearAllMMKVCache();

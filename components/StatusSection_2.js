...
const StatusSection = ({ 
 ...
 const sendStatusReply = async () => {
  ...
  // ==================== UPDATED: renderStoryCircle with 110x110 images ====================
  const renderStoryCircle = (userStatus, isLive = false) => {
    const isMyStatus = userStatus.user?.phone === currentUserPhone || userStatus.user === currentUserPhone;
    const imageUrl = getImageUrl(userStatus.statuses?.[0]?.media || userStatus.image || userStatus.broadcaster_image);
    const name = isMyStatus ? 'My Story' : (userStatus.user?.name || userStatus.broadcaster_name || userStatus.user || 'User');
    ...
    return (
      ...
      >
        <View style={styles.storyContainer}>
          {/* Story Ring - 118x118 (increased from 98) */}
          <View style={[
            styles.storyRing,
            isLive ? styles.storyRingLive : styles.storyRingStatus,
            hasUnseen && styles.storyRingUnseen,
          ]}>
            {/* Profile Image - 110x110 (increased from 90) */}
            <Image
              source={
                imageUrl
                  ? { uri: imageUrl }
            ...
            {/* Live glow ring - 126x126 (increased from 106) */}
            {isLive && (
              <Animated.View 
                style={[
                  styles.liveGlowRing,
  ...
  return (
    <View style={[styles.container, containerStyle]}>
      ...
      {loading ? (
        ...
        <View>
          {showTitle && (
            <Text style={[styles.sectionTitle, { fontWeight: '600', fontSize: 20, marginTop:5}]}>{title}</Text>
          )}
...
const createStyles = (colors, isDark, insets, customStyles = {}) => StyleSheet.create({
  ...
  storyRing: {
    width: 118, // Increased from 98
    height: 118, // Increased from 98
    borderRadius: 59, // Increased from 50
    padding: 4,
    justifyContent: 'center',
    alignItems: 'center',
    position: 'relative',
  ...
  storyImage: {
    ...
    borderRadius: 110, // Increased from 90
    borderWidth: 2.5,
    borderColor: '#fff',
    backgroundColor: '#e0e0e0',
  },
  ...
  liveGlowRing: {
    position: 'absolute',
    width: 126, // Increased from 106
    height: 126, // Increased from 106
    borderRadius: 63, // Increased from 53
    borderWidth: 3,
    borderColor: '#FF3B30',
    opacity: 0.4,
  },
